...
blogsRouter.get('/:id', async (request, response) => {
    ...
    const blogi = await Blog.findById(id).populate('user', { user: 1, name: 1, id: 1 })
    response.json(blogi)
})
blogsRouter.post('/:id/comments', async (request, response) => {
    ...
    if (blogi.comments === null || !blogi.comments) blogi.comments = []
    
    blogi.comments = blogi.comments.concat(kommentti)
    
    const palautus = await blogi.save()

    response.status(201).json(palautus.toJSON())
})
